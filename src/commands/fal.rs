...
pub async fn fal(
    ...
    match fal_res {
        Ok(_) => {
            info!(
                "Request to fal.ai recieved successfully! | Prompt: {} | Model: {}",
                prompt,
                model.to_string()
            );
            ...
            match model {
                ModelType::StableAudio => {
                    // Get the audio data
                    let audio_url = fal_res["audio_file"]["url"]
                        .as_str()
                        .unwrap_or_default()
                        .to_string();
                    let audio_filename = fal_res["audio_file"]["file_name"]
                        .as_str()
                        .unwrap_or_default()
                        .to_string();
                    elapsed_fal = None;
                ...
                _ => {
                    ...
                    elapsed_fal =
                        Some(fal_res["timings"]["inference"].as_f64().unwrap_or_default());
